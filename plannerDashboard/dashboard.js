...
async function saveAll() {
  ...
  try {
    await Promise.all([
      ...
      setDoc(profileDoc(), {...profileData, email:user.email, parkingLot:dayData.parkingLot || "", lastOpenedDay:todayKey, updatedAt:serverTimestamp()}, {merge:true})
    ]);
    markSaved();
...
function renderAll() { renderTimeline(); renderBrainDump(); renderWeekly(); renderRecurring(); renderSpecialDates(); renderGoals(); renderLastTime(); }
...
function buildTimelineItems() {
  ...
  if(c.letterDay!=="No School") {
    ...
    const schedule=SCHEDULE_BY_LETTER_DAY[c.letterDay]||[]; const last=schedule[schedule.length-1]; const closeTime=last ? addMinutes(last.time24,45) : "15:20";
  ...
  specialDatesToday().forEach(item=>add("07:00",`${item.type === "Birthday" ? "🎂" : item.type === "Anniversary" ? "♡" : "★"} ${item.name}`,item.type,`special-${item.id}`));
...
function addBrainDump() {
  const input=$("brainDumpInput"); const text=input.value.trim(); if(!text) return;
  profileData.brainDump.unshift({id:uid(),text,category:$("brainDumpCategory").value,createdAt:new Date().toISOString()});
  input.value=""; renderBrainDump(); queueSave(); showToast("Captured. You do not need to hold it in your head.");
}
...
function moveBrainItem(id,destination) {
  ...
  if(destination==="today") dayData.customItems.push({id:uid(),title:item.text,time24:"12:00",type:"From Brain Dump"});
  if(destination==="parking") dayData.parkingLot=`${dayData.parkingLot?dayData.parkingLot+"\n":""}• ${item.text}`;
  ...
  profileData.brainDump=profileData.brainDump.filter(i=>i.id!==id); $("parkingLot").value=dayData.parkingLot||""; renderAll(); queueSave(); showToast(`Moved to ${destination}.`);
}
...
function specialDatesToday() { const d=new Date(todayKey+"T12:00:00"); return (profileData.specialDates||[]).filter(i=>Number(i.month)===d.getMonth()+1&&Number(i.day)===d.getDate()); }
function daysUntilSpecial(i) { const now=new Date(todayKey+"T12:00:00"); let next=new Date(now.getFullYear(),Number(i.month)-1,Number(i.day),12); if(next<now) next=new Date(now.getFullYear()+1,Number(i.month)-1,Number(i.day),12); return Math.round((next-now)/86400000); }
