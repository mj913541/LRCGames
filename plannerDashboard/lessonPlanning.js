...
const LessonPlanner = (() => {
  ...
  function legacyLessonPlannerDoc() {
    // Legacy per-login planner path, still inside plannerDashboardUsers,
    // matching the same migration pattern used by workout.js.
    return doc(db, "plannerDashboardUsers", auth.currentUser.uid, "lessonPlanning", "progress");
  }
  ...
  async function initData() {
    ...
    try {
      ...
      // One-time migration from the signed-in user's older UID-based planner path.
      if (!snap.exists()) {
        ...
        if (legacy.exists() && legacy.data()?.grades) {
          await setDoc(lessonPlannerDoc(), {
            ...
            migratedFrom: `plannerDashboardUsers/${current.uid}/lessonPlanning/progress`,
