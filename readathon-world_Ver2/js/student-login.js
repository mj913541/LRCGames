...
function init() {
  ...
  // Restore last grade selection (optional)
  const lastGrade = localStorage.getItem("readathonV2_lastGrade");
  if (lastGrade !== null && lastGrade !== "") {
    const g = Number(lastGrade);
    if (!Number.isNaN(g) && g >= 0 && g <= 5) state.grade = g;
  }
  ...
  if (state.grade !== null) {
    // auto-continue to homerooms when coming back
    selectGrade(state.grade).catch(() => {});
  }
}
...
function wireSchoolDialog() {
  ...
  els.schoolDialog.addEventListener("close", () => {
    ...
    if (els.schoolDialog.returnValue === "ok") {
      ...
      localStorage.removeItem("readathonV2_lastGrade");
...
async function selectGrade(gradeNum) {
  ...
  localStorage.setItem("readathonV2_lastGrade", String(gradeNum));
